...
export function buildSlashCommandToolDescription(commands: SlashCommandDefinition[]): string {
  const header =
    'Execute a custom slash command defined in .gambit/commands (project) or ~/.gambit/commands (user). Use only when a listed command directly matches the task; the result is rendered command content to follow.'
