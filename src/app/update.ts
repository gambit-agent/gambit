...
export function buildWindowsUpdateScript(
  ...
  lines.push('}')
  lines.push('')
  lines.push('function Invoke-DownloadText {')
  lines.push('  param([string]$Url)')
  lines.push('  $response = Invoke-WebRequest -UseBasicParsing -Uri $Url')
  lines.push('  if ($response.Content -is [string]) {')
  lines.push('    return $response.Content')
  lines.push('  }')
  lines.push('  if ($response.Content -is [byte[]]) {')
  lines.push('    return [System.Text.Encoding]::UTF8.GetString([byte[]]$response.Content)')
  lines.push('  }')
  lines.push('  if ($response.Content -is [System.Array]) {')
  lines.push('    return [System.Text.Encoding]::UTF8.GetString([byte[]]$response.Content)')
  lines.push('  }')
  lines.push('  return [string]$response.Content')
  lines.push('}')
  lines.push('')
  lines.push('function Get-FileSha256 {')
  lines.push('  param([string]$Path)')
  lines.push('  $stream = [System.IO.File]::OpenRead($Path)')
  lines.push('  try {')
  lines.push('    $sha256 = [System.Security.Cryptography.SHA256]::Create()')
  lines.push('    try {')
  lines.push('      $hashBytes = $sha256.ComputeHash($stream)')
  lines.push("      return ([System.BitConverter]::ToString($hashBytes)).Replace('-', '').ToLowerInvariant()")
  lines.push('    } finally {')
  lines.push('      $sha256.Dispose()')
  lines.push('    }')
  lines.push('  } finally {')
  lines.push('    $stream.Dispose()')
  lines.push('  }')
  lines.push('}')
  lines.push('')
  ...
  lines.push('')
  lines.push('$updateSucceeded = $false')
  lines.push('try {')
  lines.push('  $manifestJson = Invoke-DownloadText -Url $manifestUrl')
  ...
  lines.push('  }')
  lines.push('  $updateSucceeded = $true')
  lines.push('} catch {')
  lines.push('  Log "ERROR: $($_.Exception.Message)"')
  lines.push('  throw')
  lines.push('} finally {')
  lines.push('  if ($updateSucceeded) {')
